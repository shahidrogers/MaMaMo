...
export function validateRunPack(runPack) {
    for (const key of REQUIRED_RUN_PACK_KEYS) {
        if (!(key in runPack)) {
            throw new Error(`Run pack missing required key "${key}"`);
        }
    }
...
export class StructuralModelEngine {
    ...
    lag(currentQuarter, seriesName, periods = 1) {
        ...
        for (let i = 0; i < periods; i += 1) {
            ...
            q = quarter === 1 ? `${year - 1}Q4` : `${year}Q${quarter - 1}`;
        }
    ...
    run(stepFn) {
        ...
        for (const quarter of this.quarters) {
            const state = stepFn({
                ...
                previousQuarter: periods => {
                    let q = quarter;
                    for (let i = 0; i < periods; i += 1) q = nextQuarterBack(q);
                    return q;
                },
            });
...
function nextQuarterBack(quarter) {
    const { year, quarter: q } = parseQuarter(quarter);
